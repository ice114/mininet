...
from TopoMaker0322 import MakeSatSwitchTopo
...
class ctrlSat(object):
    ...
    def genDevices(self):
        ...
        self.groud_ending_host.append(
            Host('u2', mac=self.genMac(8, 8), ip=self.genIpforHost(8, 8), locat=[109, 18])) #sanya
        self.groud_ending_host.append(
            Host('u3', mac=self.genMac(9, 9), ip=self.genIpforHost(9, 9), locat=[-77, 39]))  #washingtion DC
    ...
    def genLink(self):
        ...
        # for i in range(self.planeNum):
        #     for j in range(self.satPerplane):
        #         self.satSwitches[i][j].addLink1(self.hostLinkToSwitches[i][j].name,'eth1')
        #         self.hostLinkToSwitches[i][j].addLink1(self.satSwitches[i][j].name,'eth0')
        #         self.linksInfo.append(self.genLinkInDetail(self.satSwitches[i][j].name,
        #                                                    self.hostLinkToSwitches[i][j].name,
        #                                                    'eth1',
        #                                                    'eth0'))

        #s11 link s21 in port2  s21 link s11 in port3
        ...
        #s11 link s12 in port4 s12 link s11 in port5
        for i in range(self.planeNum):
            for j in range(self.satPerplane-1):
                self.satSwitches[i][j].addLink1(self.satSwitches[i][j+1].name,4)
                self.satSwitches[i][j+1].addLink1(self.satSwitches[i][j].name,5)
                self.linksInfo.append(self.genLinkInDetail(self.satSwitches[i][j].name,
                                                           self.satSwitches[i][j+1].name,
                                                           4,
                                                           5))
            self.satSwitches[i][0].addLink1(self.satSwitches[i][self.satPerplane-1].name,5)
            self.satSwitches[i][self.satPerplane-1].addLink1(self.satSwitches[i][0].name,4)
    ...
    def genLocatDict(self):
        ...
        for i in range(len(self.groud_ending_host)):
            ...
            data=[self.groud_ending_host[i].ipAddress,self.groud_ending_host[i].locat[0],self.groud_ending_host[i].locat[1]]
        ...
        for i in range(len(self.hostLinkToSwitches)):
            #for j in range(len(self.hostLinkToSwitches[i])):
                ...
                data = [self.hostLinkToSwitches[i].ipAddress, self.hostLinkToSwitches[i].locat[1],
                        self.hostLinkToSwitches[i].locat[0]]
