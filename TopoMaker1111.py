...
class MakeSatSwitchTopo(Topo):
    def __init__(self, sw_path, json_path, thrift_port, pcap_dump,designedTopo, **opts):
        ...
        self.addLink(self.gw_list[0], self.host_list[0], 1, 1)
        self.addLink(self.switch_list[2][2],self.host_list[1],3,1)

        for i in range(len(self.gw_list)):
            gw_port=2
            self.addLink(self.gw_list[i], self.gw_h[i], 0, 0)
        ...
        for i in range(len(self.link_s2s)):
            ...
            #self.addLink(self.link_s2s[i].sw1, self.link_s2s[i].sw2)
