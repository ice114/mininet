...
class MakeSatSwitchTopo(Topo):
    def __init__(self, sw_path, json_path, thrift_port, pcap_dump,designedTopo, **opts):
        ...
        for i in range(len(designedTopo.satSwitches)):
            ...
            for j in range(len(designedTopo.satSwitches[i])):
                sw=self.addSwitch(designedTopo.satSwitches[i][j].name,
                                  ip=designedTopo.satSwitches[i][j].ipAddress,
                                sw_path=sw_path,
                               json_path=json_path,
        ...
        for i in range(len(designedTopo.hostLinkToSwitches)):
            tp_hs=[]
            tp_hnames=[]
            #for j in range(len(designedTopo.hostLinkToSwitches[i])):
            h=self.addHost(designedTopo.hostLinkToSwitches[i].name,
                            ip=designedTopo.hostLinkToSwitches[i].ipAddress,
                            mac=designedTopo.hostLinkToSwitches[i].macAddress)
            #tp_hnames.append(designedTopo.hostLinkToSwitches[i].name)
            print(designedTopo.hostLinkToSwitches[i].name,designedTopo.hostLinkToSwitches[i].ipAddress,designedTopo.hostLinkToSwitches[i].macAddress)
            self.host_list.append(h)
            self.host_name.append(designedTopo.hostLinkToSwitches[i].name)
