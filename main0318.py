...
from TopoMaker0318 import MakeSatSwitchTopo
...
class ctrlSat(object):
    ...
    def genDevices(self):
        ...
        self.hostLinkToSwitches.append(
            Host('h'+ str(11),mac=self.genMac(1,1),ip=self.genIpforHost(1,1),locat=[116,40]))  #beijing  ground station
        self.hostLinkToSwitches.append(
            Host('h' + str(22), mac=self.genMac(2, 2), ip=self.genIpforHost(2, 2), locat=[109, 18])) #sanya
        self.hostLinkToSwitches.append(
            Host('h' + str(33), mac=self.genMac(3, 3), ip=self.genIpforHost(3, 3), locat=[-77, 39]))  #washingtion DC
    ...
    def genLink(self):
        ...
        for i in range(self.planeNum):
            for j in range(self.satPerplane):
                for k in range(len(self.hostLinkToSwitches)):
                    self.hostLinkToSwitches[k].addLink(self.satSwitches[i][j].name)
                    self.satSwitches[i][j].addLink(self.hostLinkToSwitches[k].name)
                    self.linksInfo_h2s.append(self.genLinkInDetail(self.hostLinkToSwitches[k].name,
                                                                   self.satSwitches[i][j].name,
                                                                   self.hostLinkToSwitches[k].portSum,
    ...
    def start(self):
        ...
        self.genLinkMap()
        #self.route_fouding()
        self.locatDict=self.genLocatDict2()
        self.makeTopo()
