...
from TopoMaker1111 import MakeSatSwitchTopo
...
class ctrlSat(object):
    ...
    def genDevices(self):
        ...
        self.gw_host.append(
            Host('gw1', mac=self.genMac(9,9),ip=self.genIpforHost(9,9),locat=[117,41]))
        ...
        # )
        # self.gw_host.append(
        #     Host('gw2', mac=self.genMac(10, 10), ip=self.genIpforHost(10, 10), locat=[76, 40]))
    ...
    def genLink(self):
        ...
        for i in range(self.planeNum - 1):
            for j in range(self.satPerplane):
                self.satSwitches[i][j].addLink1(self.satSwitches[i + 1][j].name, 4)
                self.satSwitches[i + 1][j].addLink1(self.satSwitches[i][j].name, 5)
