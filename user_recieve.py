...
def send_data1():
    ...
    if (ap_b_long != 0 and ap_b_lati != 0):
        ...
        # print(self_locat)
        accept_sat = distance_calc.find_shortest_path(trans2ten(int(self_locat[1])), trans2ten(int(self_locat[2])))[-3:]
        Note1 = open('sat_log/reciever_accept_sat.txt', mode='a')
        ...
        pkt = Ether(src=get_if_hwaddr(iface), dst='ff:ff:ff:ff:ff:ff', type=4864)
        pkt = pkt / replyData(direction=0, d_longitude=d_longitude, d_lagitude=d_lagitude)
        pkt = pkt / IP(dst=addr)
        # pkt.show2()
        bf=time.time()*10000
        sendp(pkt, iface=str(send_iface), verbose=False)
        ...
        counter += 1
        send_data_7 = (bf+af)/2
        Note = open('sat_log/7_send_data.txt', mode='a')

        Note.writelines([str(int(round(send_data_7))), "\n"])
...
def handle_pkt(pkt):
    ...
    if IP in pkt:
        ...
        if IPinfo.src!=src_ip:
            ...
            if myData in pkt:
                    rec_data_6=time.time()
                    # print("got a data packet:", datetime.datetime.now())
                    print("got a data packet:", time.clock())
                    time_end=time.time()

                    Note = open('sat_log/6_rec_data.txt', mode='a')
                    ...
                    print("data_rec counting:",data_count_rec)
