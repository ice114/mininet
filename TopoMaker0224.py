    ...
    def __init__(self, sw_path, json_path, thrift_port, pcap_dump,designedTopo, **opts):
        ...
        for i in range(len(designedTopo.hostLinkToSwitches)):
            ...
            for j in range(len(designedTopo.hostLinkToSwitches[i])):
                h=self.addHost(designedTopo.hostLinkToSwitches[i][j].name,
                             ip=designedTopo.hostLinkToSwitches[i][j].ipAddress,
                             mac=designedTopo.hostLinkToSwitches[i][j].macAddress)
                tp_hs.append(h)
                tp_hnames.append(designedTopo.hostLinkToSwitches[i][j].name)
            self.host_list.append(tp_hs)
            self.host_name.append(tp_hnames)
    ...
    def findHost(self,name):
        for i in range(len(self.host_list)):
            for j in range(len(self.host_list[i])):
                if self.host_name[i][j]==name:
                    return self.host_list[i][j]
