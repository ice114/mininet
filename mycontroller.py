...
sys.path.append(
    os.path.join('/home/fxy/Workspace/P4/tutorials/utils'))
...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw,portlist, tunnel_id,
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        action_params={
            "port": portlist[str(egress_sw.name)]
        })
    ingress_sw.WriteTableEntry(table_entry)
...
def write_tunnel_ingress(p4info_helper,ingress_sw,tunnel_id,dst_ip_addr):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyIngress.ipv4_lpm",
        match_fields={
            "hdr.ipv4.dstAddr": (dst_ip_addr, 32)
    ...
    ingress_sw.WriteTableEntry(table_entry)
    print
    "Installed ingress tunnel rule on %s" % ingress_sw.name

def write_tunnel_forward(p4info_helper, ingress_sw,egress_sw,portlist,tunnel_id):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyIngress.myTunnel_exact",
        match_fields={
            "hdr.myTunnel.dst_id": tunnel_id
    ...
    ingress_sw.WriteTableEntry(table_entry)
    print
    "Installed transit tunnel rule on %s" % ingress_sw.name

def write_tunnel_egress(p4info_helper, egress_sw, tunnel_id,dst_eth_addr):
    table_entry = p4info_helper.buildTableEntry(
        table_name="MyIngress.myTunnel_exact",
        match_fields={
            "hdr.myTunnel.dst_id": tunnel_id
    ...
    egress_sw.WriteTableEntry(table_entry)
    print
    "Installed egress tunnel rule on %s" % egress_sw.name
...
def main(p4info_file_path, bmv2_file_path):
    ...
    s12_port={}
    s13_port = {}
    s11_port.update({"s13":3})
    s11_port.update({"s12":3})
    s12_port.update({"s13":4})
    s12_port.update({"h12":1})
    ...
    try:
        ...
        s13.SetForwardingPipelineConfig(p4info=p4info_helper.p4info,
                                        bmv2_json_file_path=bmv2_file_path)
        print "Installed P4 Program using SetForwardingPipelineConfig on s13"
        # Write the rules that tunnel traffic from h1 to h2
        writeTunnelRules(p4info_helper, ingress_sw=s11, egress_sw=s13, portlist=s11_port,tunnel_id=100,
                         dst_eth_addr="08:00:00:00:01:03", dst_ip_addr="10.0.1.3")
        write_tunnel_ingress(p4info_helper,ingress_sw=s11,tunnel_id=100,dst_ip_addr="10.0.1.3")
        write_tunnel_forward(p4info_helper,ingress_sw=s11,egress_sw=s13,portlist=s11_port,tunnel_id=100)
        write_tunnel_forward(p4info_helper,ingress_sw=s11,egress_sw=s13,portlist=s12_port,tunnel_id=100)
        write_tunnel_egress(p4info_helper,egress_sw=s13,tunnel_id=100,dst_eth_addr="08:00:00:00:01:03")
        #writeTunnelRules(p4info_helper, ingress_sw=s11, egress_sw=s13, portlist=s12_port, tunnel_id=100,
        #                 dst_eth_addr="08:00:00:00:01:03", dst_ip_addr="10.0.1.3")
        #writeTunnelRules(p4info_helper, ingress_sw=s11, egress_sw=s13, portlist=s13_port, tunnel_id=100,
        #                 dst_eth_addr="08:00:00:00:01:03", dst_ip_addr="10.0.1.3")
        # Write the rules that tunnel traffic from h2 to h1
        #writeTunnelRules(p4info_helper, ingress_sw=s2, egress_sw=s1, portlist=s2_port,tunnel_id=200,
                         #dst_eth_addr="08:00:00:00:01:11", dst_ip_addr="10.0.1.1")

        # TODO Uncomment the following two lines to read table entries from s1 and s2
        readTableRules(p4info_helper, s11)
        readTableRules(p4info_helper, s12)
        readTableRules(p4info_helper, s13)
       # readTableRules(p4info_helper, s2)
        ...
        while True:
            ...
            printCounter(p4info_helper, s11, "MyIngress.egressTunnelCounter", 100)
            printCounter(p4info_helper, s12, "MyIngress.ingressTunnelCounter", 100)
            printCounter(p4info_helper, s12, "MyIngress.egressTunnelCounter", 100)
            printCounter(p4info_helper, s13, "MyIngress.ingressTunnelCounter", 100)
            printCounter(p4info_helper, s13, "MyIngress.egressTunnelCounter", 100)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p4info', help='p4info proto in text format from p4c',
                        ...
                        default='./build/extending_tunneling.p4.p4info.txt')
    parser.add_argument('--bmv2-json', help='BMv2 JSON file from p4c',
                        type=str, action="store", required=False,
                        default='./build/extending_tunneling.json')
