...
parser MyParser(packet_in packet,
    ...
    state parse_ethernet {
        ...
        TYPE_DATA:parse_myData;
        TYPE_RDATA:parse_myRData;
        }
    ...
    state parse_myData {
        packet.extract(hdr.myData);
        transition accept;
    }
    state parse_myRData {
        packet.extract(hdr.myRData);
        transition accept;
    }
...
control MyIngress(inout headers hdr,
    ...
    action srcRoute_finish() {
        ...
        if(hdr.myData.isValid()){
            hdr.myData.direction=2;
        }
        if(hdr.myRData.isValid()){
            hdr.myRData.direction=2;
        }
    }
    ...
    apply {
        if (hdr.srcRoutes[0].isValid()){
            ...
            else if (hdr.myData.isValid()){
                accept_forward();
            }
            else if (hdr.myRData.isValid()){
                accept_forward();
            }
        }
...
control MyDeparser(packet_out packet, in headers hdr) {
    apply {
        ...
        packet.emit(hdr.myData);
        packet.emit(hdr.myRData);
