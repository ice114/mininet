...
class testtopo(Topo):
    def __init__(self, sw_path, json_path, **opts):
        ...
        self.addSwitch('s1', sw_path=sw_path, json_path=json_path, thrift_port=9090, device_id=1, pcap_dump=False)
        self.addSwitch('s2', sw_path=sw_path, json_path=json_path, thrift_port=9091, device_id=2, pcap_dump=False)
        self.addHost('h1')
        self.addHost('h2')
        self.addLink('s1','s2')
        self.addLink(('s1','h1'))
...
def gen_topo2():
    ...
    net.stop()
def gen_topo3():
    topo=testtopo(sw_path, json_path)
    net = Mininet(topo=topo,
                  host=P4Host,
                  switch=P4RuntimeSwitch,
                  controller=None)
